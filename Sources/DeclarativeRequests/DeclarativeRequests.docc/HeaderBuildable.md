...
``AcceptHeader``, ``ContentTypeHeader``, ``UserAgentHeader``, ``HostHeader``,
``OriginHeader``, ``RefererHeader``, ``AcceptLanguageHeader``,
``AcceptEncodingHeader``, ``CustomHeader``, and the values returned by
``AuthorizationHeader`` and by ``Header/setValue(_:)`` / ``Header/addValue(_:)``.
