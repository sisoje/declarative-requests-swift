...
**The whole point of this package is ergonomics.** Every rule below — one spelling per capability, capability-or-frequency, no machinery — exists to serve the most ergonomic way to declare a request. Rules are means; if a rule ever fights ergonomics at a call site, ergonomics wins and the rule gets rewritten.
...
**Rules of thumb:** popularity decides what enters the framework — how frequently a feature is used in real-world requests is the bar for adding it; niche needs stay on the escape hatches (`RequestMutation`, raw `RequestBlock`) or in a separate package. A new block should be a one-liner over `RequestState`/`RequestMutation`. If a feature needs its own thread, run loop, or a second result builder, it does not go in this package — `RequestBuilder` is the only builder; `MultipartPart` lists are a second *content kind* of the same builder (contextual overloads, the SwiftUI ContentBuilder model). A new content kind is warranted only when the element **cannot** be a `RequestState` transform — parts mean something only relative to sibling parts. Cookies, headers and query items are transforms; they list and accumulate in the one context, and never get a grouping builder (`Headers {}` died for this). When in doubt, the original version of this library (branch `stara-verzija`) did everything essential in ~540 lines — that is the spirit level.
...
- **`RequestBlock`** (`Syntax/RequestBlock.swift`) — the leaf every node reduces to, and itself declarable syntax: the escape hatch ({ state in }) and the group ({ blocks }) — the leaf. Wraps a `RequestStateTransformClosure` directly; its `body` is unreachable (traps if called). All other blocks ultimately reduce to this.
- **`RequestState`** (`State/RequestState.swift`) — the mutable context threaded through every block during a build. Stores exactly one `URLRequest` (the source of truth) plus the `JSONEncoder` used by `Encodable`-driven blocks. `path`, `queryItems`, `cookies`, and `encodedBodyItems` are computed projections that read and write through the request — one narrow public spelling per concern; the omnibus `urlComponents` is private plumbing. Never add stored side-state next to the request. `setBaseURL(_:)` is an operation, not state: it combines the base into `request.url` with no readback (combination consumes the base/path boundary).
...
- **`RequestStateTransformClosure`** = `(RequestState) throws -> Void`. The currency (internal — goes public if anyone ever needs user-defined scoped modifiers).
