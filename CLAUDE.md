...
- Never catch-and-rewrap an error the system already threw — `catch { throw ourError(reason: error.localizedDescription) }` destroys the type and userInfo. Propagate. Typed `DeclarativeRequestsError` cases exist only where WE detect the failure and there is no underlying error.
