...
- **`RequestBlock`** (`Blocks/RequestBlock.swift`) — the leaf. Wraps a `RequestStateTransformClosure` directly; its `body` is unreachable (traps if called). All other blocks ultimately reduce to this. **Construction is pure description** — the builder init stores its closure and evaluates it at build, so constructing a block never runs anything and never throws; all evaluation, including `throw` statements in builder bodies, happens at `.request`.
