...
- **`Endpoint`** paths are relative to the base: it strips a leading `/` at write time (`Endpoint("/v1/users")` == `Endpoint("v1/users")`; bare `/` resets to root) and sets `urlComponents.path`, replacing any previous path. `BaseURL` application is pure RFC 3986 resolution — a base of `https://host/api/` is a directory prefix: `/api/v1/users`. Contract: end the base path with `/`; a slashless base path (`…/api`) is treated by RFC as a file and dropped — documented, not defended.
