...
- **`URLRequest` properties typed as Foundation enums get NO block** — `RequestMutation[\.cachePolicy, .returnCacheDataElseLoad]` is the spelling; type inference gives leading-dot cases for free. Wrapping such a property in a named block is duplication (CachePolicy and NetworkServiceType existed once; both were deleted).
