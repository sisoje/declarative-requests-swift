...
- **`BaseURL` order doesn't matter** — it resolves *components* against a base URL, preserving any path/query already declared. `Endpoint` *replaces* the path; `Path` does RFC 3986 reference resolution (urljoin) — bare segments append (treating the base path as a directory), leading `/` resets to root, `..`/`.` traverse.
